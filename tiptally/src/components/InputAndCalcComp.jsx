...
useEffect(() => {
  ...
  if (!isNaN(parsedBill) && !isNaN(parsedNumberOfPeople)) {
...
useEffect(() => {
  ...
  if (!isNaN(parsedBill) && !isNaN(parsedSelectedTip) && !isNaN(parsedNumberOfPeople)) {
    let calculatedValue = 0;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', width: '50%', marginRight: '1rem' }}>
      ...
      <div style={{ textAlign: 'left', display: 'flex', flexDirection: 'column', marginTop: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 30%)', gap: '1rem' }}>
          {tipVal.map((tip) => (
            <Button key={tip} variant="contained" style={{ backgroundColor: 'hsl(183, 100%, 15%)', marginTop: '1rem' }} onClick={(e) => handleTipSlelect(e, tip)}>
              {tip} %
            </Button>
          ))}
          <input type="number" placeholder="Custom" style={{ fontFamily: 'Space Mono',  backgroundColor: 'hsl(189, 41%, 97%)', border: '0', borderRadius: '.4rem', height: '2.3rem', width: '5.7rem', marginTop: '1rem' , textAlign: 'right' , padding: '0 0.5rem'}} onChange={handleCustomBill} value={customTip}/>
        </div>
      </div>
